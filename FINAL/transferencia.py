        ...
        while True:
            ...
            if len(myresult): #si existe la cuenta
                if myresult[0][0] > int(datos['monto']):
                    ...
                    data_search = (datos['cuentadestino'],)
                    cur.execute(sql,data_search)
                    myresult = cur.fetchall()
                    saldo = myresult[0][0]
                    ...
                    sql = "INSERT INTO TransaccionEnviada (Monto, Comentario, Fecha, Cuenta_idCuenta, Numero_cuenta_destino) VALUES (?, ?, ?, ?, ?)"  #inserta en envio
                    val = ( datos['monto'], datos['comentario'], datos['fecha'], datos['cuenta'], datos['cuentadestino'],)
                    cur.execute(sql, val)
                    conn.commit()

                    sql = "INSERT INTO TransaccionRecibida (Monto, Comentario, Fecha, Cuenta_idCuenta, Numero_cuenta_origen) VALUES (?, ?,?, ?, ?)"  #inserta en recibido
